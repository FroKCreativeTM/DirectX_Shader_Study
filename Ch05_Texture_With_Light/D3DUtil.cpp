...
void RenderScene()
{
	...
	// 쉐이더 전역변수들을 설정
	g_pLightingShader->SetMatrix("g_WorldMatrix", &matWorld);
	g_pLightingShader->SetMatrix("g_ViewMatrix", &matView);
	g_pLightingShader->SetMatrix("g_ProjectionMatrix", &matProjection);

	g_pLightingShader->SetVector("g_WorldLightPosition", &g_WorldLightPosition);
	g_pLightingShader->SetVector("g_WorldCameraPosition", &g_WorldCameraPosition);

	// 텍스처를 입힌다.
	g_pLightingShader->SetVector("g_LightColor", &g_LightColor);
	g_pLightingShader->SetTexture("DiffuseMap_Tex", g_pStoneDM);
	g_pLightingShader->SetTexture("SpecularMap_Tex", g_pStoneSM);
...
{
	// 쉐이더 로딩
	g_pLightingShader = LoadShader("SpecularMapping.fx");
