...
// 쉐이더
LPD3DXEFFECT			g_pEnvironmentShader	= nullptr;
// 조명의 위치
...
LPDIRECT3DCUBETEXTURE9	g_pEnvironmentTexture = nullptr;	// 배경을 위한 텍스쳐
// 빛의 색상
...
void RenderScene()
{
	...
	// 쉐이더 전역변수들을 설정
	g_pEnvironmentShader->SetMatrix("g_WorldMatrix", &matWorld);
	g_pEnvironmentShader->SetMatrix("g_WorldViewProjectionMatrix", &matWorldViewProjection);

	g_pEnvironmentShader->SetVector("g_WorldLightPosition", &g_WorldLightPosition);
	g_pEnvironmentShader->SetVector("g_WorldCameraPosition", &g_WorldCameraPosition);

	// 텍스처를 입힌다.
	g_pEnvironmentShader->SetVector("g_LightColor", &g_LightColor);
	g_pEnvironmentShader->SetTexture("DiffuseMap_Tex", g_pStoneDM);
	g_pEnvironmentShader->SetTexture("SpecularMap_Tex", g_pStoneSM);
	g_pEnvironmentShader->SetTexture("NormalMap_Tex", g_pStoneNM);
	g_pEnvironmentShader->SetTexture("EnvironmentMap_Tex", g_pEnvironmentTexture);

	// 쉐이더를 시작한다.
	// 앞으로 그릴 모든 물체에 이 쉐이더를 적용할 것이란 뜻
	UINT numPasses = 0;
	g_pEnvironmentShader->Begin(&numPasses, NULL);
	{
		for (UINT i = 0; i < numPasses; ++i)
		{
			g_pEnvironmentShader->BeginPass(i);
			{
				// 구체를 그린다.
				g_pTeapot->DrawSubset(0);
			}
			g_pEnvironmentShader->EndPass();
		}
	}
	// 이 쉐이더를 적용하는 것을 끝내라
	g_pEnvironmentShader->End();
}
...
{
	// 쉐이더 로딩
	g_pEnvironmentShader = LoadShader("EnvironmentMapping.fx");
	if (!g_pEnvironmentShader)
	{
		return false;
	}

	// 모델 로딩
	g_pTeapot = LoadModel("TeapotWithTangent.x");
	if (!g_pTeapot)
	{
		return false;
	}
	...
	D3DXCreateCubeTextureFromFile(gpD3DDevice, "Snow_ENV.dds", &g_pEnvironmentTexture);
	if (!g_pEnvironmentTexture)
	{
		return false;
	}
...
void Cleanup()
{
	...
	if (g_pEnvironmentShader)
	{
		g_pEnvironmentShader->Release();
		g_pEnvironmentShader = nullptr;
	}
	...
	if (!g_pEnvironmentTexture)
	{
		g_pEnvironmentTexture->Release();
		g_pEnvironmentTexture = nullptr;
	}
